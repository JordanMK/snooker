...
const BASE_URL = (function () {
  ...
  if (!url || !port) {
    throw new Error(
      'env variables NEXT_PUBLIC_API_BASE_URL and NEXT_PUBLIC_API_PORT must be set'
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('response', (response) => {
      if (response.statusCode === 200) {
        ...
        reject(
          new Error(
            `Failed to retrieve klassement for speeldagen with id ${id}`
          )
        );
      }
    });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!response.ok) {
        return reject(
          new Error(
            `Request to ${apiUrl} failed with ${response.status} ${response.statusText}`
          )
        );
      }
      ...
      reject(
        new Error(`Request to ${apiUrl} failed with error ${error.message}`)
      );
    }
  });
}

export function postSpeeldagJokerAndSchiftingsAntwoord(
  jokerGebruikt,
  schiftingsAntwoord,
  speeldagId
) {
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 201) {
          ...
          reject(
            new Error(
              `Failed to post joker and schiftingsantwoord. Status code: ${res.statusCode} ${responseData}`
            )
          );
        }
      });
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 201) {
          ...
          reject(
            new Error(
              `Failed to Post speeldag vote. Status code: ${res.statusCode}`
            )
          );
        }
      });
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 201) {
          ...
          reject(
            new Error(
              `Failed to post wedstrijd. Status code: ${res.statusCode}`
            )
          );
          resolve([]);
        }
...
export async function patchWedstrijd(
  date,
  thuis,
  uit,
  resultaat,
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', async () => {
        ...
        if (res.statusCode === 201) {
          ...
          console.error(
            'Failed to patch wedstrijd:',
            new Error(
              `Failed to patch wedstrijd. Status code: ${res.statusCode}`
            )
          );
          reject(
            new Error(
...
export function updateKlassementen(seizoenId) {
  return new Promise((resolve, reject) => {
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', async () => {
        // Using async for better handling
        if (res.statusCode === 200) {
          try {
            const seizoenen = await getSpeeldagen();
            if (seizoenen[0]) {
              await Promise.all(
                seizoenen.map((speeldag) =>
          ...
          reject(
            new Error(
              `Failed to post klassement. Status code: ${res.statusCode}`
            )
          );
        }
      });
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          ...
          reject(
            new Error(
              `Failed to update speeldag klassement. Status code: ${res.statusCode}`
            )
          );
        }
      });
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 201) {
          ...
          reject(
            new Error(`Failed to post speeldag. Status code: ${res.statusCode}`)
          );
          resolve([]);
        }
      });
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 204) {
          ...
          reject(
            new Error(
              `Failed to delete wedstrijd. Status code: ${res.statusCode}`
            )
          );
        }
      });
    ...
    const req = request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 201) {
          ...
          reject(
            new Error(
              `Failed to put speeldag vote. Status code: ${res.statusCode}`
            )
          );
        }
      });
...
export function getUserVotesBySpeeldagId(speeldagId) {
  ...
  return new Promise((resolve, reject) => {
    const request = get(
      `${speeldagVotesUrl}${speeldagId}/${loggedInUser}/votes`
    );
    request.on('response', (response) => {
      if (response.statusCode === 200) {
        let data = '';
...
export async function updateSpeeldagIsOnline(speeldagId, isOnline) {
  try {
    const response = await fetch(`${speeldagenUrl}${speeldagId}/isOnline`, {
      method: 'PUT', // Aangezien we de status updaten
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    console.error('Error updating speeldag status:', error);
    throw error;
  }
}
